...
function createObservablePropSpy() {
    var subject = createReplaySubject();
    var observableSpy = rxjs_1.defer(function () { return subject; });
...
function addObservableHelpers(objectToDecorate, providedSubject, 
// eslint-disable-next-line @typescript-eslint/no-empty-function
onSubjectConfiguredCallback) {
    if (onSubjectConfiguredCallback === void 0) { onSubjectConfiguredCallback = function noop() { }; }
    ...
    objectToDecorate.nextWithValues = function nextWithValues(valuesConfigs) {
        ...
        if (valuesConfigs && valuesConfigs.length > 0) {
            var results$ = merge_subject_with_default_values_1.mergeSubjectWithDefaultValues(providedSubject, valuesConfigs);
        ...
        if (valueConfigsPerCall && valueConfigsPerCall.length > 0) {
            ...
            valueConfigsPerCall.forEach(function (valueConfiguration) {
                ...
                if (valueConfiguration.delay) {
                    returnedObservable = returnedObservable.pipe(operators_1.delay(valueConfiguration.delay));
                }
                if (!valueConfiguration.doNotComplete) {
                    returnedObservable = returnedObservable.pipe(operators_1.take(1));
                }
                /* istanbul ignore else */
