    ...
    if (methodsToSpyOnOrConfig) {
        if (Array.isArray(methodsToSpyOnOrConfig)) {
            ...
            if (methodsToSpyOnOrConfig.providedMethodNames) {
                ...
                methodsToSpyOn = __spread(methodsToSpyOn, methodsToSpyOnOrConfig.providedMethodNames);
            }
        }
    }
    if (methodsToSpyOn.length > 0) {
        methodNames.push.apply(methodNames, __spread(methodsToSpyOn));
    }
    var autoSpy = {};
    if (observablePropsToSpyOn.length > 0) {
        observablePropsToSpyOn.forEach(function (observablePropName) {
            autoSpy[observablePropName] = _1.createObservablePropSpy();
        });
    }
    _1.createAccessorsSpies(autoSpy, gettersToSpyOn, settersToSpyOn, accessorSpyFactory);
